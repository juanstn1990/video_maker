...
import type { VideoClipConfig, ImageFitMode, TextAnimation } from '../../types/video'
...
interface Props { clip: VideoClipConfig }

export function VideoClipPanel({ clip }: Props) {
  const {
    updateVideoClip, updateTransition, config,
    addTextOverlay, removeTextOverlay, updateTextOverlay,
  } = useEditorStore()
  const fps = config.fps
  const durationSec = clip.durationFrames / fps
  const maxTrimStart = Math.max(0, clip.durationSeconds - 1)
  const [expandedOverlay, setExpandedOverlay] = useState<string | null>(null)

  return (
    <div className="space-y-4 p-3">
      <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Clip de Video</h3>

      {/* Duration */}
      <Slider
        label="Duración"
        min={fps * 0.5}
        max={Math.max(fps * 30, clip.durationFrames)}
        step={fps * 0.5}
        value={clip.durationFrames}
        displayValue={`${Math.round(durationSec * 10) / 10}s`}
        onChange={(e) => updateVideoClip(clip.id, { durationFrames: parseInt(e.target.value) })}
      />

      {/* Audio */}
      <div className="border-t border-gray-800 pt-3 space-y-2">
        <h4 className="text-xs text-gray-500 uppercase tracking-wider">Audio del video</h4>

        <label className="flex items-center gap-2 text-xs text-gray-300 cursor-pointer">
          <input
            type="checkbox"
            checked={clip.muted}
            onChange={(e) => updateVideoClip(clip.id, { muted: e.target.checked })}
        ...
        {!clip.muted && (
          <Slider
            ...
            step={0.05}
            value={clip.volume}
            displayValue={`${Math.round(clip.volume * 100)}%`}
            onChange={(e) => updateVideoClip(clip.id, { volume: parseFloat(e.target.value) })}
          />
        )}
      </div>
      ...
      {maxTrimStart > 0 && (
        <div className="border-t border-gray-800 pt-3 space-y-2">
          <h4 className="text-xs text-gray-500 uppercase tracking-wider">Recorte</h4>
          <Slider
            label="Iniciar desde"
            min={0}
            max={maxTrimStart}
            ...
            displayValue={`${clip.startFromSeconds.toFixed(1)}s`}
            onChange={(e) => updateVideoClip(clip.id, { startFromSeconds: parseFloat(e.target.value) })}
          />
        </div>
      )}
      ...
      {/* Fit mode */}
      <div className="border-t border-gray-800 pt-3 space-y-2">
        <h4 className="text-xs text-gray-500 uppercase tracking-wider">Ajuste de imagen</h4>
        <div className="grid grid-cols-3 gap-1">
          {([
            { value: 'cover', label: 'Recortar', desc: 'Llena el encuadre (recorta)' },
            { value: 'contain', label: 'Completa', desc: 'Video completo (barras negras)' },
            ...
            <button
              ...
              onClick={() => updateVideoClip(clip.id, { fitMode: opt.value })}
            >
              {opt.label}
            </button>
          ))}
